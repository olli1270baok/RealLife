...
const MODULES = ['ueberlastung', 'kindeswohl', 'strafanzeige', 'mietmangel'];
...
function buildModuleUI() {
  $('#moduleFields').innerHTML = MODULE_FIELDS[currentModule];
  bindInputs();
}
...
function switchModuleUI(mod) {
  document.body.classList.remove('ueberlastung-active', 'kindeswohl-active', 'strafanzeige-active', 'mietmangel-active');
  document.body.classList.add(mod + '-active');
  $$('.module-chip').forEach(c => c.classList.toggle('active', c.dataset.mod === mod));
  // Update module title
  const iconMap = { ueberlastung: '🚨', kindeswohl: '🛡️', strafanzeige: '🚔', mietmangel: '🏠' };
...
function applyI18n() {
  ...
  $('#moduleTitleText').textContent = t(titleMap[currentModule]);
  ...
  $$('.module-chip').forEach(c => {
    ...
    c.lastChild.textContent = t(map[k]);
  });
  ...
  $('#moduleStamp').textContent = stampText[currentModule];
...
function renderRecipients() {
  ...
  if (roleSel) {
    ...
    const prefix = rolePrefix[currentModule];
...
function renderEvidence() {
  ...
  if (!list) return;
  list.innerHTML = '';
  EVIDENCE[currentModule].forEach(key => {
